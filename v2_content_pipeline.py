...
# ─── MEDIUM & HASHNODE CREDENTIALS ────────────────────────────
MEDIUM_INTEGRATION_TOKEN = os.getenv("MEDIUM_INTEGRATION_TOKEN")
HASHNODE_API_KEY = os.getenv("HASHNODE_API_KEY")
HASHNODE_PUBLICATION_ID = os.getenv("HASHNODE_PUBLICATION_ID")  # From your Hashnode blog settings
...
def generate_platform_summary(title, keyword, excerpt, post_url, article_content):
    """Use Claude to generate a 400-500 word summary for Medium and Hashnode"""
    print("   ✍️  Generating platform summary...")

    client = anthropic.Anthropic(api_key=os.getenv("ANTHROPIC_API_KEY"))

    prompt = f"""Write a 400-500 word summary article for Medium and Hashnode based on this full article.
...
- Summarise the key points from the full article in 3-4 paragraphs
- Do NOT reproduce the full article — this is a teaser summary
...
- Do not include a title — just the body text starting from the first paragraph
...
Write the summary now:"""
    ...
    try:
        ...
        print(f"   ✅ Platform summary generated ({len(summary)} chars)")
        return summary
    except Exception as e:
        print(f"   ⚠️  Summary generation error: {e}")
        # Fallback — use excerpt plus CTA
        return f"{excerpt}\n\nRead the full review at [Equinoxen Media]({post_url})"


# ─── POST TO MEDIUM ────────────────────────────────────────────
def post_to_medium(title, summary_markdown, post_url, keyword):
    """Post summary article to Medium via API"""
    if not MEDIUM_INTEGRATION_TOKEN:
        print("   ⚠️  MEDIUM_INTEGRATION_TOKEN missing — skipping Medium")
        return False
    ...
    try:
        ...
        if user_response.status_code != 200:
            print(f"   ❌ Medium auth failed: {user_response.status_code} — {user_response.text[:200]}")
            return False
        ...
        post_data = {
            ...
            "content": f"# {title}\n\n{summary_markdown}",
            "tags": ["SaaS", "Software", "Business", "Productivity", keyword[:20]],
            "publishStatus": "public",
            "canonicalUrl": post_url,  # tells Medium the original source — prevents duplicate content issues
        }
        ...
        if response.status_code in [200, 201]:
            medium_url = response.json().get("data", {}).get("url", "")
            print(f"   ✅ Medium posted: {medium_url}")
            return medium_url
        else:
            print(f"   ❌ Medium failed: {response.status_code} — {response.text[:200]}")
            return False
...
# ─── POST TO HASHNODE ──────────────────────────────────────────
def post_to_hashnode(title, summary_markdown, post_url, keyword):
    """Post summary article to Hashnode via GraphQL API"""
    if not HASHNODE_API_KEY or not HASHNODE_PUBLICATION_ID:
        print("   ⚠️  HASHNODE_API_KEY or HASHNODE_PUBLICATION_ID missing — skipping Hashnode")
        return False

    print("   📤 Posting to Hashnode...")

    # Convert markdown to HTML for Hashnode (it accepts both but HTML is more reliable)
    content_markdown = f"{summary_markdown}"
    ...
    variables = {
        "input": {
            ...
            "originalArticleURL": post_url,  # canonical URL back to Equinoxen
            "tags": [],
            "metaTags": {
                "title": title,
                "description": f"Read our expert {keyword} review at Equinoxen Media.",
            },
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   ❌ Hashnode failed: {response.status_code} — {response.text[:200]}")
            return False
...
def post_to_social(title, excerpt, post_url, keyword="", article_content="",
                   category_id=1, image_url=None, pinterest_image_url=None, article_position=0):
    ...
    # ── MEDIUM & HASHNODE ────────────────────────────────────
    if post_url:
        summary = generate_platform_summary(title, keyword, excerpt, post_url, article_content)
        time.sleep(2)

#DEBUG        post_to_medium(title, summary, post_url, keyword)
#        time.sleep(2)

        post_to_hashnode(title, summary, post_url, keyword)
        time.sleep(2)
...
- Examples of strong title words: Proven, Powerful, Essential, Complete, Exposed, Dominate, Game-Changing
...
def publish_to_wordpress(title, content, metadata, category_id, draft=True, featured_image_id=None):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            # Push Rank Math focus keyword via registered REST field
...
def run_pipeline(num_articles=3, publish_as_draft=False, publish_to_wp=True):
    ...
    while published_count < num_articles and opp_index < len(opportunities):
        ...
        # ── POST TO SOCIAL MEDIA + MEDIUM + HASHNODE ─────────────
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "list":
        ...
        run_pipeline(
            num_articles=2,         # How many articles to generate
