...
from content_pipeline import (
    ...
    clear_rating, clear_research, save_research,
)
...
def detect_content_type(title):
    ...
    if " vs " in title_lower or " vs. " in title_lower:
        return "comparison"
    if title_lower.startswith("best ") or title_lower.startswith("top "):
        return "buying_guide"
    return "review"
...
def retrofit_post(post, forced_type=None):
    ...
    # ── Step 2.5: clear stale caches so this retrofit's fresh audit ─────
    # actually gets used, instead of apply_all_rubric_tables silently
    # reusing an old cached score from before this verification pass.
    ...
    # ── Step 4.5: seed the research cache with what this audit just ────
    # verified, so content_pipeline.py's next article about this product
    # doesn't pay for a redundant search on facts we just confirmed.
    for p in products_to_clear:
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "--list":
        ...
        result = retrofit_post(post, forced_type=forced_type)
        if result:
            print(f"\n{'=' * 60}")
